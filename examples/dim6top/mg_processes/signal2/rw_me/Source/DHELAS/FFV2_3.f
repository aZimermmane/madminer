...
C     
     ...
     $ * W3))
      V3(3)= DENOM*(-CI)*(F2(5)*F1(3)+F2(6)*F1(4)-P3(0)*OM3*TMP14)
      V3(4)= DENOM*(-CI)*(-F2(6)*F1(3)-F2(5)*F1(4)-P3(1)*OM3*TMP14)
      V3(5)= DENOM*(-CI)*(-CI*(F2(6)*F1(3))+CI*(F2(5)*F1(4))-P3(2)*OM3
     $ *TMP14)
      V3(6)= DENOM*(-CI)*(-F2(5)*F1(3)-P3(3)*OM3*TMP14+F2(6)*F1(4))
      END
...
C     Gamma(3,2,-1)*ProjM(-1,1)
C     
      SUBROUTINE FFV2_8_3(F1, F2, COUP1, COUP2, M3, W3,V3)
      IMPLICIT NONE
      COMPLEX*16 CI
      PARAMETER (CI=(0D0,1D0))
      COMPLEX*16 COUP1
      ...
      CALL FFV8_3(F1,F2,COUP2,M3,W3,VTMP)
      DO I = 3, 6
        V3(I) = V3(I) + VTMP(I)
      ENDDO
      END
