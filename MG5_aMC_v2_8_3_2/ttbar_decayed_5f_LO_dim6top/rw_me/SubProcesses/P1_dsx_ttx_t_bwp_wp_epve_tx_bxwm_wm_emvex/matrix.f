      SUBROUTINE M24_SMATRIXHEL(P,HEL,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     
      INTEGER USERHEL
      COMMON/M24_HELUSERCHOICE/USERHEL
C     ----------
C     BEGIN CODE
C     ----------
      USERHEL=HEL
      CALL M24_SMATRIX(P,ANS)
      USERHEL=-1

      END

      SUBROUTINE M24_SMATRIX(P,ANS)
C     
...
C     
      ...
      COMMON/M24_BORN_BEAM_POL/POLARIZATIONS
      DATA ((POLARIZATIONS(I,J),I=0,NEXTERNAL),J=0,5)/NPOLENTRIES*-1/

C     
C     FUNCTIONS
C     
      LOGICAL M24_IS_BORN_HEL_SELECTED
...
C      only three external particles.
     ...
     $       -1.AND.(.NOT.M24_IS_BORN_HEL_SELECTED(IHEL))) THEN
              CYCLE
            ENDIF
            T=M24_MATRIX(P ,NHEL(1,IHEL),JC(1))
            IF(POLARIZATIONS(0,0).EQ.
     $       -1.OR.M24_IS_BORN_HEL_SELECTED(IHEL)) THEN
              ANS=ANS+T
            ENDIF
      ...
      REAL*8 FUNCTION M24_MATRIX(P,NHEL,IC)
C     
C     Generated by MadGraph5_aMC@NLO v. 2.8.3.2, 2021-02-02
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     Amplitude(s) for diagram number 1
      CALL FFV10_3_0(W(1,4),W(1,2),W(1,7),GC_867,GC_750,AMP(1))
C     Amplitude(s) for diagram number 2
      CALL FFV2_4_0(W(1,4),W(1,2),W(1,7),GC_922,GC_931,AMP(2))
      CALL FFV2_4_3(W(1,1),W(1,5),GC_919,GC_930,MDL_MW,MDL_WW,W(1,7))
C     Amplitude(s) for diagram number 3
      CALL FFV10_3_0(W(1,4),W(1,2),W(1,7),GC_867,GC_750,AMP(3))
C     Amplitude(s) for diagram number 4
      CALL FFV2_4_0(W(1,4),W(1,2),W(1,7),GC_922,GC_931,AMP(4))
      ...
      DO I = 1, NCOLOR
        ...
        ENDDO
        M24_MATRIX = M24_MATRIX+ZTEMP*DCONJG(JAMP(I))/DENOM(I)
      ENDDO

      END

      SUBROUTINE M24_GET_VALUE(P, ALPHAS, NHEL ,ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     the include file with the values of the parameters and masses 
      ...
      SUBROUTINE M24_INITIALISEMODEL(PATH)
C     ROUTINE FOR F2PY to read the benchmark point.    
      IMPLICIT NONE
      CHARACTER*512 PATH
CF2PY INTENT(IN) :: PATH
      ...
      LOGICAL FUNCTION M24_IS_BORN_HEL_SELECTED(HELID)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     ----------
      ...
      DO I=1,NEXTERNAL
        ...
        ENDDO
        IF(.NOT.FOUNDIT) THEN
          M24_IS_BORN_HEL_SELECTED = .FALSE.
          RETURN
        ENDIF
      ENDDO
